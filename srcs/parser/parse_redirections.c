...
/*   Updated: 2022/09/19 15:47:18 by znajda        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
t_lexer	*find_first_none_direct(t_l_p_pack pack)
{
	t_lexer	*search;

	search = NULL;
	if (pack.to_search->token_type > Lesser)
		search = pack.to_search;
	return (search);
}
...
t_l_p_pack	handle_redirections(t_l_p_pack pack)
{
	...
	if (!pack.to_search)
		return (pack);
	search = find_first_none_direct(pack);
	pack.no_file = 0;
	while (pack.to_search && pack.to_search->token_type != Pipe)
	{
